...
{
    public partial class DGMileageReport : System.Web.UI.Page
    {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(strConnectionString))
                {
                    ...
                    for (int i = 0; i < size; i++)
                    {
                        ...
                        row.Add("stime", dt.Rows[i]["starttime"].ToString());
                        row.Add("etime", dt.Rows[i]["endtime"].ToString());
                        row.Add("litres", dt.Rows[i]["litres"].ToString());
                        jArray.Add(row);
