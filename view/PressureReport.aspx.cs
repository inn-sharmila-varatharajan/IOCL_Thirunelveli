...
{
    public partial class PressureReport : System.Web.UI.Page
    {
        ...
        public static string getReport(string fromdate, string ToDate)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(strConnectionString))
                {
                    ...
                    cmd.Parameters.AddWithValue("@from", "ptx");
                    ...
                    for (int i = 0; i < size; i++)
                    {
                        ...
                        row.Add("day", dt.Rows[i]["day"].ToString());
                        row.Add("hour", dt.Rows[i]["hour"].ToString());
                        row.Add("voltage", dt.Rows[i]["voltage"].ToString());
                        jArray.Add(row);
